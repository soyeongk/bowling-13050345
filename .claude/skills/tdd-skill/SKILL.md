...
## 이 프로젝트의 체크포인트 워크플로우 (PLAN → RED → GREEN → REVIEW)

이 저장소에서는 기본 Red-Green-Refactor 사이클에 사람 파트너의 검토 체크포인트 두 곳과 그에 따른 커밋/푸시 시점이 추가된다. **계획은 테스트 코드보다 먼저 작성한다.**

1. **PLAN**: 다음으로 어떤 동작(behavior)을 검증할지, 어떤 테스트를 작성할지, 그 테스트를 통과시키기 위해 프로덕션 코드를 어떻게 구현할지를 `Plan.md`(또는 유사한 계획 문서)에 적고, 사람 파트너에게 검토를 요청한다. **계획이 승인되기 전에는 테스트 코드도, 프로덕션 코드도 작성하지 않는다.**
2. **RED**: 승인된 계획대로 실패하는 테스트를 작성하고, 실패하는 것을 직접 확인한다 (아래 "RED 검증" 그대로).
3. **RED 체크포인트 커밋 & 푸시**: 승인된 계획 + 실패하는 테스트를 커밋하고 푸시한다.
4. **GREEN**: 승인된 계획대로 구현하고, 테스트가 실제로 통과하는지 확인한다 (아래 "GREEN 검증" 그대로). 계획에 없던 것을 임의로 추가하지 않는다.
5. **REVIEW**: 사람 파트너가 GREEN 단계의 결과 코드를 리뷰한다. `Plan.md`에 없던 사항이 구현되지는 않았는지 확인하고, 필요하면 리팩터링을 요청한다.
6. **REVIEW 체크포인트 커밋 & 푸시**: 리뷰가 끝나고(필요한 리팩터링까지 반영된 뒤) 커밋하고 푸시한다.

**핵심:** 커밋 & 푸시는 이 두 체크포인트(RED 단계 완료 시, REVIEW 단계 완료 시)에서만 수행한다 — 그 사이 임의 시점에 커밋하지 말 것. 계획 승인 없이 테스트 코드를 작성하지 말 것. 계획 승인 없이 GREEN으로 넘어가지 말 것.
...
digraph tdd_cycle {
    rankdir=LR;
    plan [label="PLAN\n다음 동작/테스트/구현\n계획 작성", shape=box, style=filled, fillcolor="#ffe6cc"];
    approve [label="사람 파트너\n승인?", shape=diamond];
    red [label="RED\n실패하는 테스트 작성", shape=box, style=filled, fillcolor="#ffcccc"];
    verify_red [label="제대로\n실패하는지 확인", shape=diamond];
    commit_red [label="커밋 & 푸시\n(RED 체크포인트)", shape=box, style=filled, fillcolor="#eeeeee"];
    green [label="GREEN\n최소한의 코드", shape=box, style=filled, fillcolor="#ccffcc"];
    verify_green [label="통과 확인\n전체 그린", shape=diamond];
    refactor [label="REFACTOR\n정리", shape=box, style=filled, fillcolor="#ccccff"];
    review [label="REVIEW\n사람 파트너가\n계획 대비 검토", shape=box, style=filled, fillcolor="#ffe6cc"];
    commit_review [label="커밋 & 푸시\n(REVIEW 체크포인트)", shape=box, style=filled, fillcolor="#eeeeee"];
    next [label="다음", shape=ellipse];

    plan -> approve;
    approve -> red [label="승인"];
    approve -> plan [label="반려"];
    red -> verify_red;
    ...
    verify_green -> review;
    review -> commit_review [label="승인"];
    review -> refactor [label="리팩터링\n요청"];
    commit_review -> next;
    next -> plan;
}
```

이 저장소에서는 위 "이 프로젝트의 체크포인트 워크플로우" 절이 실제 진행 순서다. 아래 각 단계(PLAN/RED/GREEN/REFACTOR) 설명은 그 안에서 "무엇을 어떻게 하는가"에 대한 세부 가이드로 계속 적용된다.

### PLAN - 계획 작성 및 승인 (테스트 코드보다 먼저)

다음으로 검증할 동작(behavior)이 무엇인지, 어떤 테스트를 작성할지, 그 테스트를 통과시키기 위해 프로덕션 코드를 어떻게 구현할지를 `Plan.md`에 짧게 적는다. 사람 파트너가 검토하고 승인할 때까지 테스트 코드도, 프로덕션 코드도 작성하지 않는다.
...
RED 검증까지 끝났다면, 승인된 계획 + 실패하는 테스트를 커밋 & 푸시한다 (RED 체크포인트). 그 다음에만 GREEN으로 넘어간다.
...
GREEN(및 필요한 REFACTOR)이 끝나면, 사람 파트너가 결과 코드를 `Plan.md`와 대조하며 리뷰한다:
- 계획에 없던 것이 구현되지 않았는가
- 계획대로 구현되었는가
- 리팩터링이 더 필요한가 (필요하면 REFACTOR로 돌아간다)

리뷰가 끝나면 커밋 & 푸시한다 (REVIEW 체크포인트).
...
- [ ] 테스트 코드를 작성하기 전에 `Plan.md`를 작성하고 사람 파트너의 승인을 받았다
- [ ] RED 체크포인트(승인된 계획 + 실패하는 테스트)를 커밋 & 푸시했다
- [ ] 사람 파트너가 GREEN 결과를 `Plan.md`와 대조하여 REVIEW했다
- [ ] REVIEW 체크포인트를 커밋 & 푸시했다
