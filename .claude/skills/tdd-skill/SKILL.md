...
## 이 프로젝트의 체크포인트 워크플로우 (RED-PLAN → GREEN → REVIEW)

이 저장소에서는 기본 Red-Green-Refactor 사이클에 사람 파트너의 검토 체크포인트 두 곳과 그에 따른 커밋/푸시 시점이 추가된다.

1. **RED**: 실패하는 테스트를 작성하고, 실패하는 것을 직접 확인한다 (아래 "RED 검증" 그대로).
2. **PLAN**: 그 테스트를 통과시키기 위해 프로덕션 코드를 어떻게 구현할지 계획을 `Plan.md`(또는 유사한 계획 문서)로 작성하고, 사람 파트너에게 검토를 요청한다. **계획이 승인되기 전에는 프로덕션 코드를 작성하지 않는다.**
3. **RED 체크포인트 커밋 & 푸시**: 실패하는 테스트 + 승인된 계획을 커밋하고 푸시한다.
...
**핵심:** 커밋 & 푸시는 이 두 체크포인트(RED 단계 완료 시, REVIEW 단계 완료 시)에서만 수행한다 — 그 사이 임의 시점에 커밋하지 말 것. 계획 승인 없이 GREEN으로 넘어가지 말 것.
...
digraph tdd_cycle {
    ...
    plan [label="PLAN\n구현 계획 작성 +\n사람 파트너 승인", shape=box, style=filled, fillcolor="#ffe6cc"];
    ...
    red -> verify_red;
    verify_red -> plan [label="예"];
    verify_red -> red [label="잘못된\n실패"];
    plan -> commit_red [label="승인"];
    plan -> plan [label="반려"];
    commit_red -> green;
    green -> verify_green;
...
이 저장소에서는 위 "이 프로젝트의 체크포인트 워크플로우" 절이 실제 진행 순서다. 아래 각 단계(RED/GREEN/REFACTOR) 설명은 그 안에서 "무엇을 어떻게 하는가"에 대한 세부 가이드로 계속 적용된다.
...
RED 검증까지 끝났다면, 이 테스트를 통과시키기 위해 프로덕션 코드를 어떻게 구현할지 `Plan.md`에 짧게 적는다. 사람 파트너가 검토하고 승인할 때까지 GREEN으로 넘어가지 않는다. 승인되면 실패하는 테스트 + 계획을 커밋 & 푸시한다 (RED 체크포인트).
...
- [ ] RED 단계에서 `Plan.md`를 작성하고 사람 파트너의 승인을 받았다
- [ ] RED 체크포인트(테스트 + 승인된 계획)를 커밋 & 푸시했다
